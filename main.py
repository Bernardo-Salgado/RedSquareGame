...
def main():
    ...
    while True:
        for event in pygame.event.get():
            ...
            if menu_action == "start":
            ...
            elif menu_action in menu.dfs_options:

                max_depth = int(menu_action.split(": ")[1]) # Get the max depth form selected
                # Get the current board size from the menu
                cols, rows = menu.board_sizes[menu.current_board_size_index]
                game = Game(cols, rows)  # Pass the selected board size to the Game instance
                ...
                solver.track_solver(lambda: solver.dfs(max_depth), f'DFS (max depth: {max_depth})')
